...
def setup_language_model_chain(vectorstore: Chroma, topk: int):
    ...
    #print("printing runnable: ",q)
    # rag_results = []
    # for sub_question in sub_questions:
    #     retrieved_docs = retriever.get_relevant_documents(sub_question)
    #     answer = (prompt_decomposition | model | StrOutputParser()).invoke({"context": retrieved_docs,"question": sub_question})
...
def invoke_chain(chain, question, context):
    ...
    try:
        ...
        response = chain.stream({"question": question, "context": context})
